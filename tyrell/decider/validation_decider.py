...
class ValidationDecider(ExampleDecider):
    ...
    def traverse_regex(self, node: Node):
        ...
        assert self.split_valid is None or len(node.children) == len(
            self.split_valid[0]), f"{len(node.children)} and {len(self.split_valid[0])}"
    ...
    def traverse_regex_rec(self, node, tree_idx):
        ...
        if production.is_function():
            if production.name == "concat":
                ...
                #  that must occur.
                if self.always_matches_examples(st) and '[' not in st and st not in self.already_must_occur:
                    self.already_must_occur.add(st)
                    new_predicates.append(Predicate("char_must_occur", [char_node, tree_idx]))
    ...
    def never_matches_examples(self, regex):
        return not any(map(lambda ex: re.search(regex, ex.input[0]) is not None, self.valid_exs))

    def always_matches_examples(self, regex):
        return all(map(lambda ex: re.search(regex, ex.input[0]) is not None, self.valid_exs))
